...
DECLARE_SOA_TABLE(HfCandD0Fulls, "AOD", "HFCANDD0FULL",
                  ...
                  hf_cand::Chi2PCA,
                  hf_cand_2prong::KfTopChi2OverNdf,
...
struct HfTreeCreatorD0ToKPi {
  ...
  void init(InitContext const&)
  {
    ...
    // if ((doprocessDataWithDCAFitterN && doprocessDataWithKFParticle) || ((doprocessMcWithDCAFitterOnlySig || doprocessMcWithDCAFitterOnlyBkg || doprocessMcWithDCAFitterAll)&& (doprocessMcWithKFParticleOnlySig || doprocessMcWithKFParticleOnlyBkg || doprocessMcWithKFParticleAll))) {
    //   LOGP(fatal, "treeCreator: Only one process function between DCAFitterN and KFParticle can be enabled at a time.");
    // }
    // if ((doprocessDataWithDCAFitterN || doprocessDataWithKFParticle) && (doprocessMcWithDCAFitterOnlySig || doprocessMcWithDCAFitterOnlyBkg || doprocessMcWithDCAFitterAll || doprocessMcWithKFParticleOnlySig || doprocessMcWithKFParticleOnlyBkg || doprocessMcWithKFParticleAll)) {
    //   LOGP(fatal, "treeCreator: Only one process function between processData and processMc can be enabled at a time.");
    // }
  }
                   ...
                   TracksWPid const&, aod::BCs const&)
  {
    ...
    for (auto const& candidate : candidates) {
      ...
      if constexpr (ReconstructionType == o2::aod::hf_cand::useKFParticle) {
        ...
        topolChi2PerNdf = candidate.kfTopChi2OverNdf();
      }
      else {
                 ...
                 TracksWPid const&, aod::BCs const&)
  {
    ...
    for (const auto& candidate : reconstructedCandSets) {
      ...
      double eD = eD0(candidate);
      double ctD = ctD0(candidate);
      if constexpr (ReconstructionType == o2::aod::hf_cand::useDCAFitterN) {
        if (candidate.isSelD0()) {
      ...
      } else if constexpr (ReconstructionType == o2::aod::hf_cand::useKFParticle) {
        if (candidate.isSelD0()) {
          fillTable(candidate, prong0, prong1, 0, candidate.kfGeoMassD0(), cosThetaStarD0(candidate), candidate.kfTopChi2OverNdf(), ctD, yD, eD, candidate.flagMcMatchRec(), candidate.originMcRec());
        }
        if (candidate.isSelD0bar()) {
          fillTable(candidate, prong0, prong1, 1, candidate.kfGeoMassD0bar(), cosThetaStarD0bar(candidate), candidate.kfTopChi2OverNdf(), ctD, yD, eD, candidate.flagMcMatchRec(), candidate.originMcRec());
        }
      }
    }
