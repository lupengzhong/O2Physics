...
struct HfCandidateCreator2Prong {
  ...
  OutputObj<TH1F> hKFMassFailed{TH1F("hKFMassFailed", ";inv. mass with topo fit failed;entries", 500, 0., 5.)}; // 0 for D0 failed, 1 for D0bar failed
  OutputObj<TH1F> hMass2Top{TH1F("hMass2Top", "2-prong candidates;inv. mass (#pi K) (GeV/#it{c}^{2});entries", 600, -1.0, 5.)};
                              ...
                              aod::BCsWithTimestamps const& bcWithTimeStamps)
  {
    ...
    for (const auto& rowTrackIndexProng2 : rowsTrackIndexProng2) {
      ...
      // KFParticle kfPosPion(kfpTrackPosPi, kPiPlus);
      // KFParticle kfNegPion(kfpTrackNegPi, kPiPlus);
      // KFParticle kfPosKaon(kfpTrackPosKa, kKPlus);
      // KFParticle kfNegKaon(kfpTrackNegKa, kKPlus);
      ...
      if (fillHistograms) {
        ...
        if (constrainKfToPv) {
          float massD0Top = 0., errMassD0Top = 0.;
          if (kfCandD0Topol2PV.GetMass(massD0Top, errMassD0Top))
            hKFMassFailed->Fill(massD0); // Set up D0 topological constraints to PV failed
          hMass2Top->Fill(massD0Top);
        }
      }
    }
  }
